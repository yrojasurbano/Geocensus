    ...
    template: `
        <div class="w-full bg-white rounded-lg p-5 flex flex-col h-full">
            <div class="flex justify-between items-start mb-2">
                <div>
                    <h3 class="text-xl font-bold text-gray-800">Total Nacional | <span class="text-[#C2264B]">Distribución de la población por sexo y Edad</span>
                    </h3>
                    ...
                    @for (group of data(); track group.ageRange) {
                        <div class="grid grid-cols-[1fr_40px_1fr] items-center gap-4 group hover:bg-gray-50 rounded transition-colors py-[1px]">
                            ...
                            <div class="flex justify-end h-3 relative w-full items-center">
                                <span class="mr-2 text-[9px] text-gray-400 opacity-0 group-hover:opacity-100 transition-opacity">{{ group.male }}
                                    %</span>
                                <div class="h-full bg-[#5A9CF8] rounded-l-sm transition-all duration-500 hover:brightness-90"
                                     [style.width.%]="(group.male / maxVal()) * 100"></div>
                            </div>
                            ...
                            <div class="flex justify-start h-3 relative w-full items-center">
                                <div class="h-full bg-[#D45D79] rounded-r-sm transition-all duration-500 hover:brightness-90"
                                     [style.width.%]="(group.female / maxVal()) * 100"></div>
                                <span class="ml-2 text-[9px] text-gray-400 opacity-0 group-hover:opacity-100 transition-opacity">{{ group.female }}
                                    %</span>
                            </div>
                        </div>
                    }
