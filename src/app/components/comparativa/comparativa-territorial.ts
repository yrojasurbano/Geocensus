...
export interface FilaDep {
    departamento: string;
    superficie:   number;   // se mantiene en la interfaz para cálculos internos
    poblacion:    number;
    hombres:      number;
    mujeres:      number;
    razon:        number;
    densidad:     number;   // se mantiene en la interfaz para cálculos internos
    pctUrbano:    number;
    edadMediana:  number;
    p65:          number;
    pct65:        number;
        ...
        <section
          ...
          <span class="flex items-center gap-1.5 text-[10px] xl:text-[11px] font-black text-[#0056a1] uppercase tracking-widest">
            ...
            {{ filasTabla().length }} registro{{ filasTabla().length !== 1 ? 's' : '' }}
                        ...
                        <!-- Totales rápidos — oculto en xs -->
                        <div class="hidden sm:flex items-center gap-3 text-[10px]">
                            <span class="text-gray-500">Total: <strong class="text-[#0056a1]">{{ fmtN(totales().pob) }}</strong></span>
                            <span class="text-[#0056a1]">H: <strong>{{ fmtN(totales().hom) }}</strong></span>
                            <span class="text-[#33b3a9]">M: <strong>{{ fmtN(totales().muj) }}</strong></span>
                        </div>
                    </div>
                    ...
                    <div class="overflow-auto flex-1 min-h-0 -webkit-overflow-scrolling-touch">
                        <table class="w-full text-[13px] xl:text-[14px] 2xl:text-[15px] border-collapse" style="min-width:600px">
                            ...
                            <tr>
                                ...
                                <!-- Indicadores Demográficos (2 cols: Razón, Edad Mediana) -->
                            ...
                            <tbody>
                                ...
                                @for (fila of filasTabla(); track $index; let even = $even) {
                                    <tr
                                            ...
                                            <!-- Departamento -->
                                    <td class="px-3 py-2 font-semibold text-[#002d5c] whitespace-nowrap border-r border-gray-100 text-[13px]">
                  <span class="flex items-center gap-1.5">
                    <span class="w-1.5 h-1.5 rounded-full bg-[#0056a1] shrink-0"></span>
                      {{ fila.departamento }}
                  </span>
                                    ...
                                    <!-- Provincia -->
                                    @if (nivelActivo() === 'Provincial' || nivelActivo() === 'Distrital') {
                                        <td class="px-3 py-2 text-gray-700 whitespace-nowrap border-r border-gray-100 text-[13px]">
                                    ...
                                    <!-- Distrito -->
                                    @if (nivelActivo() === 'Distrital') {
                                        <td class="px-3 py-2 text-gray-600 whitespace-nowrap border-r border-gray-100 text-[13px]">
                                            {{ asDist(fila).distrito }}
                                        </td>
                                    }

                                    <!-- Población total -->
                                    <td class="px-3 py-2 text-right font-mono font-bold text-gray-800 whitespace-nowrap border-r border-gray-100 text-[13px]">
                                        {{ fmtN(fila.poblacion) }}
                                    </td>

                                    <!-- Hombres -->
                                    <td class="px-3 py-2 text-right font-mono text-[#0056a1] whitespace-nowrap border-r border-gray-100 text-[13px]">
                                        {{ fmtN(fila.hombres) }}
                                    </td>

                                    <!-- Mujeres -->
                                    <td class="px-3 py-2 text-right font-mono text-[#33b3a9] whitespace-nowrap border-r border-gray-100 text-[13px]">
                                        {{ fmtN(fila.mujeres) }}
                                    </td>
                  ...
                  <span
                          ...
                          [style.background-color]="fila.razon >= 100 ? 'rgba(52,59,159,0.13)' : 'rgba(51,179,169,0.13)'"
                          [style.color]="fila.razon >= 100 ? '#343b9f' : '#1d8c84'">
                    {{ fmtR(fila.razon) }}
                  </span>
                                    </td>

                                    <!-- Edad mediana -->
                                    <td class="px-3 py-2 text-right font-mono text-gray-700 whitespace-nowrap border-r border-gray-100 text-[13px]">
                  ...
                  <span
                          class="inline-block px-2 py-0.5 rounded-md text-[12px] font-bold"
                          [style.background-color]="fila.pct65 >= 10 ? 'rgba(45,163,176,0.15)' : fila.pct65 >= 7 ? 'rgba(248,189,19,0.18)' : 'rgba(156,163,175,0.18)'"
                          [style.color]="fila.pct65 >= 10 ? '#1d8c84' : fila.pct65 >= 7 ? '#9a7000' : '#4b5563'">
                    {{ fmtPct(fila.pct65) }}
                  </span>
                                    </td>
                            ...
                            @if (filasTabla().length > 1) {
                                <tfoot>
                                <tr class="border-t-2 border-[#0056a1]/20 bg-[#0056a1]/5 font-bold">
                                    ...
                                    <!-- Hombres total -->
                                    <td class="px-3 py-2.5 text-right font-mono text-[#0056a1] font-bold text-[13px] border-r border-gray-200">
                                        {{ fmtN(totales().hom) }}
                                    </td>
                                    <!-- Mujeres total -->
                                    <td class="px-3 py-2.5 text-right font-mono text-[#33b3a9] font-bold text-[13px] border-r border-gray-200">
                                        {{ fmtN(totales().muj) }}
                                    </td>
                                    <!-- Razón total -->
